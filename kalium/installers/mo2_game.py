...
[General]:
  ...
  gameName=<full Steam-display name MO2 plugins understand, e.g. "The Elder Scrolls V: Skyrim Special Edition">
  gamePath=<Windows path, usually Z:/... under Proton>

The selector shows the Steam name for information only.
The actual gamePath is whatever folder the user selects (or known-game path).
This keeps the Z: path untouched and makes "Switch to instance" work.
...
def normalize_mo2_game_name(name: str) -> str:
    """Return the canonical MO2 plugin name (Skyrim Special Edition etc.).

    The original Steam display name ("The Elder Scrolls V: Skyrim Special Edition")
    is kept verbatim for gamePath= (the Z: path inside Proton). Only the internal
    gameName= key is normalized so MO2 sees the right plugin.
    """
    if not name:
        return "Skyrim Special Edition"

    # Keep the exact Steam name for gamePath=; only normalize the internal key
    key = " ".join((name or "").strip().split())
    return _MO2_GAME_NAMES.get(key.lower(), (name or "").strip() or "Skyrim Special Edition")
...
def detect_managed_game_choices() -> list[ManagedGameChoice]:
    """Detect games first, then known templates. Keep detect blank for portability.

    Only known-game templates are shown by default so the user must
    explicitly pick a game from the selector (or browse) before skipping.
    """
    ...
    try:
        ...
        for g in getattr(result, "games", []) or []:
            ...
            key = mo2_name.lower()
            if key in seen_names:  # guard against duplicates
                continue
            seen_names.add(key)
    ...
    # Known templates (blank by default — user must pick explicitly)
