...
def _do_install(
    ...
    if game_name and game_path:
        try:
            ...
            # Register the real game path inside the Wine prefix (Z: mapping)
            _register_wine_game_path(
...
def _register_wine_game_path(
    prefix_path: Path,
    linux_game_path: Path,
    proton: SteamProton,
    game_name: str | None = None,
) -> None:
    """Register the game install path inside the Wine prefix via Z: mapping.

    Uses the known-games registry keys when available so tools (xEdit, LOOT,
    F4SE/SKSE, etc.) can find the game. Falls back to a generic Bethesda key
    when the game is unknown.
    """
    wine = proton.wine_binary()
    if not wine:
        log_warning("No Proton wine binary — skipping game path registry write")
        return

    linux_game_path = Path(linux_game_path).expanduser().resolve()
    # Proton/Wine maps the Linux root as Z:
    wine_game_path = "Z:" + str(linux_game_path).replace("\\", "/")

    # Prefer the real registry path/value from known_games
    reg_path = r"Software\Bethesda Softworks\Skyrim Special Edition"
    value_name = "Installed Path"
    if game_name:
        try:
            from kalium.game_finder.known_games import find_by_name, find_by_title

            known = find_by_name(game_name) or find_by_title(game_name)
            if known and known.registry_path and known.registry_value:
                reg_path = known.registry_path
                value_name = known.registry_value
        except Exception as e:
            log_warning(f"known_games lookup for registry failed: {e}")

    # Also write the Wow6432Node mirror (32-bit view)
    if reg_path.lower().startswith("software\\"):
        wow_path = "SOFTWARE\\Wow6432Node\\" + reg_path[len("Software\\") :]
    else:
        wow_path = "SOFTWARE\\Wow6432Node\\" + reg_path

    tmp = AppConfig.tmp_path()
    tmp.mkdir(parents=True, exist_ok=True)
    reg_file = tmp / "wine_game_path.reg"

    # Escape backslashes for .reg file syntax
    wine_path_reg = wine_game_path.replace("\\", "\\\\")

    reg_content = (
        "Windows Registry Editor Version 5.00\n\n"
        f"[HKEY_LOCAL_MACHINE\\{reg_path}]\n"
        f'"{value_name}"="{wine_path_reg}"\n\n'
        f"[HKEY_LOCAL_MACHINE\\{wow_path}]\n"
        f'"{value_name}"="{wine_path_reg}"\n'
    )
    ...
    try:
        subprocess.run(
            [str(wine), "regedit", str(reg_file)],
            env=env,
            check=False,
            timeout=60,
        )
        log_install(
            f"Registered game path in prefix: {reg_path}\\{value_name} = {wine_game_path}"
        )
    except Exception as e:
        log_warning(f"regedit for game path failed: {e}")
    finally:
