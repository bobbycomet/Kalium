...
def _do_install(
    ...
    # Optional USVFS 0.5.7.2 overlay (fixes virtual-folder duplication on Wine 10.20+)
    if update_usvfs:
    ...
    # Seed ModOrganizer.ini (theme + optional managed game) before first launch
    # so portable instances open without "game not found in ini".
    set_mo2_style(install_path)
    if game_name and game_path:
        try:
            from kalium.installers.mo2_game import set_mo2_managed_game

            set_mo2_managed_game(install_path, game_name, game_path, create_if_missing=True)
            ctx.log(f"Managed game: {game_name} @ {game_path}")
        except Exception as e:
