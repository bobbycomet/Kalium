...
class BackpatchGame:
    ...
    def get_target(self, version: str) -> BackpatchTarget:
        v = (version or "").strip()
        # Exact match on id or full label
        ...
        if v.lower().startswith("v"):
            ...
            for t in self.targets:
                if t.id == v2:
                    return t
        # Robust: extract a version-like token (handles UI labels like
        # "1.6.1170 Recommended for AE 1.6.x mods)" or similar mangling)
        import re
        m = re.search(r"\b(\d+\.\d+(?:\.\d+)?(?:-\w+)?)\b", v)
        if m:
            token = m.group(1)
            for t in self.targets:
                if t.id == token or t.id.startswith(token) or token.startswith(t.id):
                    return t
            # also try without trailing patch segment aliases
            for t in self.targets:
                if token in t.id or t.id in token:
                    return t
        # case-insensitive label contains
        low = v.lower()
        for t in self.targets:
            if t.id.lower() in low or low in t.label.lower():
                return t
        raise ValueError(
...
# ---------------------------------------------------------------------------
# Skyrim Special Edition (489830) — full three-depot sets for reliable
# version matching (base / core / exe). Community-canonical manifests.
# ---------------------------------------------------------------------------
_SKYRIM = BackpatchGame(
    ...
    targets=(
        BackpatchTarget(
            ...
            notes="Full three-depot set (disk + core + exe). Most common AE target.",
        ),
        BackpatchTarget(
            "1.6.1130",
            "1.6.1130",
            ...
            notes="Full three-depot set for 1.6.1130.",
        ),
        BackpatchTarget(
            "1.6.640",
            "1.6.640",
            ...
            notes="Full three-depot set for 1.6.640.",
        ),
        BackpatchTarget(
            "1.5.97",
            ...
            notes="Full three-depot set for pre-AE 1.5.97.",
        ),
    ),
)
