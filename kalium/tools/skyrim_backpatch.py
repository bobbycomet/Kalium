...
Targets: 1.6.1170, 1.6.1130, 1.6.640, 1.5.97

All targets now use the full three-depot sets (disk + core + exe) for
reliable version matching. Community-canonical manifests.
...
# Full three-depot sets (489831 disk / 489832 core / 489833 exe).
# These are the community-canonical manifests used by the major guides.
BACKPATCH_TARGETS: dict[str, list[tuple[int, int]]] = {
    ...
    "1.6.1130": [
        (489831, 3737743381894105176),
        (489832, 4341968404481569190),
        (489833, 2442187225363891157),
    ],
    "1.6.1170": [
        (489831, 8442952117333549665),
...
def normalize_target(version: str) -> str:
    ...
    if v in aliases:
        return aliases[v]
    # Robust: pull a version token out of UI labels such as
    # "1.6.1170 Recommended for AE 1.6.x mods)" or similar mangled strings
    import re
    m = re.search(r"\b(1\.[56]\.\d+(?:\.\d+)?)\b", v)
    if m:
        token = m.group(1)
        if token in BACKPATCH_TARGETS:
            return token
        # strip trailing .0 if present
        if token.endswith(".0") and token[:-2] in BACKPATCH_TARGETS:
            return token[:-2]
    # last-chance aliases by substring
    low = v.lower()
    for key in BACKPATCH_TARGETS:
        if key in low or key.replace(".", "") in low.replace(".", ""):
            return key
...
# ---------------------------------------------------------------------------
# ContentCatalog.txt — required for Skyrim SE/AE downpatches after 1.7.x
#
# After Bethesda's 1.7.99+ update the format of ContentCatalog.txt changed.
# Launching a downpatched 1.6.x binary with the new catalog crashes on load
# (std::invalid_argument on UUID-style keys). Community consensus:
# rename (or delete) the file before playing the older build.
#
# Location (Windows): %LOCALAPPDATA%\Skyrim Special Edition\ContentCatalog.txt
# On Linux / Proton that maps to:
#   <compatdata>/<appid>/pfx/drive_c/users/steamuser/AppData/Local/
#       Skyrim Special Edition/ContentCatalog.txt
# and may also appear under an MO2 instance prefix.
# ---------------------------------------------------------------------------
CONTENT_CATALOG_NAME = "ContentCatalog.txt"
CONTENT_CATALOG_BAK = "ContentCatalog.txt.bak"


def content_catalog_search_roots(game_dir: Path | None = None) -> list[Path]:
    """Candidate parent folders that may contain ContentCatalog.txt."""
    roots: list[Path] = []
    seen: set[Path] = set()
    ...
    # Native XDG / Windows-style Local AppData under $HOME
    home = Path.home()
    for rel in (
        ".local/share/Skyrim Special Edition",
        ".config/Skyrim Special Edition",
        "AppData/Local/Skyrim Special Edition",
    ):
        _add(home / rel)

    # Proton prefixes for Skyrim SE (489830) across all Steam libraries
    try:
        from kalium.steam.paths import find_all_steam_libraries, find_compatdata_for_app

        compat = find_compatdata_for_app(SKYRIM_APP_ID)
        if compat:
            _add(
                compat
                / "pfx"
    ...
    # Also scan every compatdata/*/pfx/…/Skyrim Special Edition (MO2 / other tools)
    try:
        from kalium.steam.paths import find_all_steam_libraries

        for lib in find_all_steam_libraries():
            cd_root = lib / "steamapps" / "compatdata"
            if not cd_root.is_dir():
                continue
            try:
                for child in cd_root.iterdir():
    ...
    # Near the game install (some setups mirror AppData next to the game)
    if game_dir:
        gd = Path(game_dir)
        _add(gd / "Skyrim Special Edition")
        _add(gd.parent / "Skyrim Special Edition")

    return roots


def find_content_catalog(
    game_dir: Path | str | None = None,
    explicit: Path | str | None = None,
) -> Path | None:
    """
    Locate ContentCatalog.txt.

    Prefer an explicit path (file or its parent folder). Otherwise search
    known Proton / Local AppData locations.
    """
    if explicit:
        ...
        try:
            ...
            pass
        if p.is_file() and p.name.lower() == CONTENT_CATALOG_NAME.lower():
            return p
        if p.is_dir():
            cand = p / CONTENT_CATALOG_NAME
            if cand.is_file():
                return cand
        # User may have pointed at the .bak already — treat as "already handled"
        if p.is_file() and p.name.lower() == CONTENT_CATALOG_BAK.lower():
            return None

    gd = Path(game_dir).expanduser() if game_dir else None
    for root in content_catalog_search_roots(gd):
        cand = root / CONTENT_CATALOG_NAME
        if cand.is_file():
            return cand
    return None


def disable_content_catalog(
    ...
    *,
    ...
    """
    Rename ContentCatalog.txt → ContentCatalog.txt.bak.

    Non-negotiable for Skyrim SE/AE downpatches after the 1.7.x catalog format
    change: the new file crashes 1.6.x binaries on load.

    Returns the path of the .bak file, or None if nothing was found/changed.
    If required=True and the live file cannot be found, raises RuntimeError
    so the UI/CLI can force the user to browse to it.
    """
    path = find_content_catalog(game_dir, catalog_path)
    if path is None:
        # Already bak'd in a known location?
        for root in content_catalog_search_roots(
            Path(game_dir).expanduser() if game_dir else None
        ):
            bak = root / CONTENT_CATALOG_BAK
            if bak.is_file():
                _status(
                    status,
                    f"ContentCatalog.txt already disabled:\n  {bak}",
                )
                return bak
        msg = (
            "ContentCatalog.txt not found.\n"
            "For Skyrim SE/AE downpatches this file MUST be renamed to "
            "ContentCatalog.txt.bak (or deleted), or the 1.6.x game will "
            "crash on load after a 1.7.x Creations update.\n\n"
            "Typical location (Proton):\n"
            "  <steam>/steamapps/compatdata/489830/pfx/drive_c/users/"
            "steamuser/AppData/Local/Skyrim Special Edition/ContentCatalog.txt\n\n"
            "Browse to the file (or its parent folder) and set the path, then retry."
        )
        if required:
    ...
    bak = path.with_name(CONTENT_CATALOG_BAK)
    if bak.exists():
        # Keep the newest bak; remove stale live file if present
        try:
            if path.stat().st_mtime >= bak.stat().st_mtime:
                bak.unlink()
            else:
                path.unlink()
                _status(
                    status,
                    f"ContentCatalog.txt already had a newer .bak — removed live file:\n  {path}",
                )
                return bak
        except OSError as e:
            raise RuntimeError(f"Could not reconcile ContentCatalog.txt / .bak: {e}") from e

    try:
        path.rename(bak)
    except OSError as e:
        raise RuntimeError(
            f"Failed to rename ContentCatalog.txt → .bak:\n  {path}\n  {e}\n"
            "Close the game / MO2 / any process that may lock the file, then retry."
        ) from e

    _status(
        status,
        f"ContentCatalog.txt disabled (required for 1.6.x):\n"
        f"  {path.name} → {bak.name}\n"
        f"  {bak}",
    )
    return bak
...
def apply_depots_to_game(
    ...
    *,
    backup: bool = True,
    status: StatusCb = None,
    content_catalog: Path | str | None = None,
    require_content_catalog: bool = True,
) -> Path | None:
    ...
    # Non-negotiable for Skyrim SE/AE: disable the post-1.7 ContentCatalog.txt
    # before (or as part of) applying historical depots. Without this the
    # 1.6.x binary crashes on load.
    disable_content_catalog(
        content_catalog,
        game_dir=game_dir,
        status=status,
        required=require_content_catalog,
    )
...
def diagnose(
    ...
    # ContentCatalog.txt status (critical for 1.6.x after 1.7.x Creations)
    cat = find_content_catalog(game_dir, content_catalog)
    if cat:
        lines.append(f"ContentCatalog.txt LIVE (must rename for 1.6.x): {cat}")
    else:
        found_bak = False
        for root in content_catalog_search_roots(game_dir):
            bak = root / CONTENT_CATALOG_BAK
            if bak.is_file():
                lines.append(f"ContentCatalog.txt already disabled: {bak}")
                found_bak = True
                break
        if not found_bak:
            lines.append(
                "ContentCatalog.txt: not found (browse to it if the game still "
                "crashes after downpatch — typically under compatdata/.../AppData/Local/"
                "Skyrim Special Edition/)"
            )
...
def run_backpatch(
    ...
    *,
    ...
    _status(
        ...
        f"ContentCatalog: {content_catalog or 'auto-detect'}",
    )
    ...
    backup_dir = apply_depots_to_game(
        ...
        require_content_catalog=require_content_catalog,
    )
